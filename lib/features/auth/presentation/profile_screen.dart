...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
    ...
    if (repo.hasPendingData) {
      final bool? confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text("Tienes votos pendientes. Se perderán si sales."),
          actions: [
  ...
  // 🔥 NUEVA FUNCIÓN: MOSTRAR QR GIGANTE
  ...
  Widget _buildMobileLayout(String userId, ImageProvider? imageProvider) {
    // Calculamos una altura "cuadrada" basada en el ancho de la pantalla para que la foto no se vea mini
    final screenWidth = MediaQuery.of(context).size.width;
    // Limitamos la altura máxima a 400 para que en tablets verticales no sea gigante
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          // 1. IMAGEN: Ahora pasamos una altura dinámica más generosa
      ...
      child: Row(
        ...
        children: [
          Expanded(
            ...
            child: Column(
              children: [
                _buildImageSection(imageProvider, height: 500), // En PC damos altura fija grande
                const SizedBox(height: 20),
                _buildLogoutButton(context),
              ],
            ),
  ...
  Widget _buildImageSection(ImageProvider? imageProvider, {required double height}) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.black, // Fondo negro estilo "Instagram" para resaltar la foto
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: Colors.grey.shade300),
      ),
      child: Stack(
        fit: StackFit.expand, // Esto asegura que el hijo ocupe todo
        children: [
          ClipRRect(
            borderRadius: BorderRadius.circular(16),
            child: imageProvider != null
                ? Image(
                    image: imageProvider,
                    // 🔥 CAMBIO CLAVE: .contain para ver la foto entera, .cover si prefieres llenar todo
                    // Al poner fondo negro, .contain queda muy profesional
                    fit: BoxFit.contain, 
  ...
  Widget _buildDataSection(String userId) {
    return Column(
      children: [
          ...
          child: Padding(
            ...
            child: Form(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("INFORMACIÓN PERSONAL", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.blueGrey)),
                  const SizedBox(height: 20),
                  _buildTextField(label: "Nombre Completo", controller: _nameController, icon: Icons.badge_outlined, enabled: _isEditing),
                  const SizedBox(height: 15),
                  _buildTextField(label: "Teléfono", controller: _phoneController, icon: Icons.phone_android, enabled: _isEditing, inputType: TextInputType.phone),
                  const SizedBox(height: 15),
                  _buildTextField(label: "Email", controller: _emailController, icon: Icons.email_outlined, enabled: false, isReadOnly: true),
                ],
              ),
            ),
          ),
        ...
        // 🔥 TARJETA QR MEJORADA (Clicable)
        GestureDetector(
          onTap: () => _showEnlargedQR(userId), // <--- ZOOM AL PULSAR
          child: Container(
            ...
            child: Row(
              children: [
                ...
                Hero( // Animación bonita al abrir
                  tag: 'qr_hero',
                  child: Container(
            ...
            child: ElevatedButton.icon(
              onPressed: () async {
                ...
                try {
                  ...
                  if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("¡Sincronizado! ✅"), backgroundColor: Colors.green));
                } catch (e) {
